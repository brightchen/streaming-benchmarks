...
import apex.benchmark.DimensionTuple.TupleAggregateEvent;
import it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap;
import it.unimi.dsi.fastutil.objects.Object2IntOpenHashMap;

/**
 * Convert from DimensionTuple to internal tuple used from dimension computation
 *
 */
public class DimensionTupleConverter implements Operator
{
  ...
  public final transient DefaultInputPort<TupleAggregateEvent> inputPort = new DefaultInputPort<TupleAggregateEvent>() {
    ...
    public void process(TupleAggregateEvent tuple)
    {
      ...
      for(String field: keyDescriptor.getFieldList()) {
        if(field.equals(DimensionTuple.ADID)) {
          key.setField(DimensionTuple.ADID, tuple.adId);
        }
        else if(field.equals(DimensionTuple.CAMPAIGNID)) {
          key.setField(DimensionTuple.CAMPAIGNID, tuple.campaignId);
        }
        else if(field.equals(DimensionTuple.EVENTTIME)) {
          key.setField(DimensionTuple.EVENTTIME, tuple.eventTime);
        }
      }

      key.setField(DimensionsDescriptor.DIMENSION_TIME, tuple.eventTime);
      ...
      aggregates.setField(DimensionTuple.CLICKS, tuple.clicks);
  ...
  public DimensionTupleConverter()
  {
  }
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DimensionTupleConverter.class);
}